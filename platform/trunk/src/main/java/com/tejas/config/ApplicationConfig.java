...
public class ApplicationConfig
{
	...
	/**
	 * @return A map where values can be of type (String, List, Map). The thing to be noted here is
	 *         that the config values are not decoded to their primitive types (int, boolean,
	 *         double) in the map.
	 */
	...
	/**
	 * @return A list consisting of objects of type (String, List, Map). The thing to be noted here
	 *         is that the config values are not decoded to their primitive types (int, boolean,
	 *         double) in the list.
	 */
	...
	/**
	 * Construct Map of configuration values from multiple values that match a key prefix. Prefix
	 * should consist of entire segments, segments are separated by periods. For the following
	 * example config:
	 * 
	 * <pre>
	 * *.foo.a = &quot;a&quot;;
	 ...
	 * <code> {foo.a="a", foo.b="b", foo.c.d="c.d", foo.c.e="c.e"} </code> call
	...
	public static void initialize(String configRoot, String domainName, String applicationGroup, String applicationName, String... overrideFiles)
	{
		synchronized (ApplicationConfig.class)
		{
			...
			TejasLog4jWrapper.getLogger(ApplicationConfig.class).info(dumpConfig());
		}
	}
